...
class MoUtility
{
    ...
    {     
        if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
            $moIP = sanitize_text_field($_SERVER['HTTP_CLIENT_IP']);
        }
        else if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            $moIP= sanitize_text_field($_SERVER['HTTP_X_FORWARDED_FOR']);
        }
        else {
            $moIP= sanitize_text_field($_SERVER['REMOTE_ADDR']);
        }
    ...
    public static function _handle_mo_check_ln($showMessage, $customerKey, $apiKey)
    {
        ...
        if (strcasecmp($content['status'], 'SUCCESS') == 0) {
            ...
            if (MoUtility::sanitizeCheck("licensePlan",$content)) {
                ...
                update_mo_option('check_ln', $content['licensePlan']);
            }
    ...
    public static function areFormOptionsBeingSaved($keyVal)
    {
        return current_user_can('manage_options')
            && self::mclv()
            && isset($_POST['option'])
            && $keyVal == sanitize_text_field($_POST['option']);
    }
    ...
    public static function is_addon_activated()
    {
        ...
        $registration_url = add_query_arg(
            ...
            remove_query_arg('addon',sanitize_text_field($_SERVER['REQUEST_URI'] ))
        );
