...
class WooCommerceCheckOutForm extends FormHandler implements IFormHandler
{
    ...
    function disable_auto_login_after_checkout($order)
    {
        if(is_user_logged_in()) {
            wp_logout();
            wp_safe_redirect(sanitize_text_field($_SERVER['REQUEST_URI']));
            exit();
        }
    }
    ...
    {
        $mobile_number = MoUtility::sanitizeCheck('billing_phone',$_POST);
        $phone = MoUtility::processPhoneNumber($mobile_number);
    ...
    {
        if(strcasecmp(MoPHPSessions::getSessionVar('user_email'), sanitize_email($_POST['billing_email']))!=0)
        {
